...
exports.countDocuments = async (req, res) => {
    try {
      const count = await customerLoan.countDocuments({ loanAmount: '10000' });
      res.json({ count });
    } catch (error) {
      console.error(error);
  ...
  exports.getLoanByAmount = async (req, res) => {
        //const today = new Date();
        //today.setDate(yesterday.getDate());
        try {
            const loanAmount = req.params.loanAmount; // Assuming that you expect a loanAmount parameter in the route
        
            // Use await with the query
            const results = await customerLoan.find({ loanAmount: loanAmount });
    ...
    exports.getLoanByStatus = async (req, res) => {
        try {
          const status = 'approved'; // Set the status to 'approved'
      
          // Use await with the query to find loans by status
          const loans = await customerLoan.find({ status: status });
